...
class ContaReceberService
{
    ...
    public static function gerarSeCompleta(Venda $venda): bool
    {
        ...
        if ($isMagalu) {
            $repasse = (float) $venda->valor_total_venda - (float) $venda->comissao - (float) ($venda->comissao_afiliado ?? 0) - (float) ($venda->subsidio_pix ?? 0);
        } else {
    ...
    public static function regenerar(Venda $venda): bool
    {
        ...
        if ($contaExistente) {
            ...
            if ($isMagalu) {
                $repasseBase = (float) $venda->valor_total_venda - (float) $venda->comissao - (float) ($venda->subsidio_pix ?? 0);
            } else {
