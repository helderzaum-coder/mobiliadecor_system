<x-filament-panels::page>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:16px;margin-top:12px;">
        ...
        <div style="flex:1;min-width:170px;background:var(--kpi-bg,#fff);border-radius:16px;padding:20px;box-shadow:0 1px 3px rgba(0,0,0,.1);border-top:4px solid {{ $totais['lucro_produto'] >= 0 ? '#10b981' : '#ef4444' }};">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <span style="font-size:11px;font-weight:700;color:{{ $totais['lucro_produto'] >= 0 ? '#10b981' : '#ef4444' }};text-transform:uppercase;letter-spacing:.5px;">Lucro Pedidos</span>
                <span style="width:32px;height:32px;border-radius:8px;background:{{ $totais['lucro_produto'] >= 0 ? 'rgba(16,185,129,.15)' : 'rgba(239,68,68,.15)' }};display:flex;align-items:center;justify-content:center;font-size:16px;">📈</span>
            </div>
        ...
        <div style="flex:1;min-width:170px;background:var(--kpi-bg,#fff);border-radius:16px;padding:20px;box-shadow:0 1px 3px rgba(0,0,0,.1);border-top:4px solid {{ $totais['margem_frete'] >= 0 ? '#10b981' : '#ef4444' }};">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <span style="font-size:11px;font-weight:700;color:{{ $totais['margem_frete'] >= 0 ? '#10b981' : '#ef4444' }};text-transform:uppercase;letter-spacing:.5px;">Margem Frete</span>
                <span style="width:32px;height:32px;border-radius:8px;background:{{ $totais['margem_frete'] >= 0 ? 'rgba(16,185,129,.15)' : 'rgba(239,68,68,.15)' }};display:flex;align-items:center;justify-content:center;font-size:16px;">📊</span>
            </div>
            ...
            <div class="rounded-xl shadow border-l-4 {{ $borderColor }} p-4 {{ $isCancelada ? 'bg-red-50 dark:bg-red-950/40' : 'bg-white dark:bg-gray-800' }}">
                ...
                <div class="flex flex-wrap items-stretch gap-2 text-xs">
                    ...
                    <div class="flex items-center px-3 py-2 rounded-lg" style="background:rgba(31,41,55,0.5);border:1px solid rgba(124,58,237,0.6);">
                        <div>
                            <div style="color:#9ca3af;font-size:10px;text-transform:uppercase;letter-spacing:0.05em;">🎟️ Cupom Vendedor{{ $venda->cupom_shopee_descricao ? ' · ' . $venda->cupom_shopee_descricao : '' }}</div>
                            <div class="font-semibold" style="color:#c084fc;">-R$ {{ number_format($cupomShopeeCard, 2, ',', '.') }}</div>
                        </div>
                    </div>
                    @endif

                    @php $cupomPlataformaCard = (float) ($venda->cupom_plataforma ?? 0); @endphp
                ...
                <div class="flex flex-wrap gap-2 mt-3">
                    @if(!$temNfe)
                        ...
                        <button onclick="let n=prompt('Número da NF-e:');if(n)@this.buscarNfePorNumero({{ $venda->id_venda }},n)"
                    ...
                    @if($isMadeiraMadeira && !$temPlanilha)
                        <button wire:click="aplicarPlanilhaMM({{ $venda->id_venda }})" wire:loading.attr="disabled"
                            style="background:#00BFFF;color:#fff;padding:3px 10px;font-size:11px;border-radius:5px;border:none;cursor:pointer;">
                            📊 Aplicar Planilha MM
                        </button>
                    @endif
                    @if($isWebcontinental && !$temPlanilha)
                        <button wire:click="aplicarPlanilhaWC({{ $venda->id_venda }})" wire:loading.attr="disabled"
                            style="background:#00FF7F;color:#fff;padding:3px 10px;font-size:11px;border-radius:5px;border:none;cursor:pointer;">
                            📊 Aplicar Planilha WC
                        </button>
                    @endif
                    ...
                    @if($isShopee)
                        ...
                        <div x-data="{ open: false, valor: '{{ $cupomShopeeVal > 0 ? number_format($cupomShopeeVal, 2, '.', '') : '' }}', desc: '{{ $venda->cupom_shopee_descricao ?? '' }}' }" style="display:inline-block;">
                            <button @click="open = !open" type="button"
                                ...
                                🎟️ Cupom Vendedor{{ $cupomShopeeVal > 0 ? ' R$ ' . number_format($cupomShopeeVal, 2, ',', '.') : '' }}
                            </button>
                            <span x-show="open" x-cloak style="display:inline-flex;align-items:center;gap:4px;margin-left:4px;">
                                <input x-model="valor" type="text" placeholder="Valor" style="width:80px;padding:3px 6px;font-size:11px;border-radius:4px;border:1px solid #4b5563;background:#1f2937;color:#fff;">
                        ...
                        @php $cupomPlataformaVal = (float) ($venda->cupom_plataforma ?? 0); @endphp
                        <div x-data="{ open: false, valor: '{{ $cupomPlataformaVal > 0 ? number_format($cupomPlataformaVal, 2, '.', '') : '' }}' }" style="display:inline-block;">
                            <button @click="open = !open" type="button"
                                style="background:{{ $cupomPlataformaVal > 0 ? '#065f46' : '#374151' }};color:#fff;padding:3px 10px;font-size:11px;border-radius:5px;border:none;cursor:pointer;">
                                🎟️ Cupom Shopee{{ $cupomPlataformaVal > 0 ? ' R$ ' . number_format($cupomPlataformaVal, 2, ',', '.') : '' }}
                            </button>
                            <span x-show="open" x-cloak style="display:inline-flex;align-items:center;gap:4px;margin-left:4px;">
                                <input x-model="valor" type="text" placeholder="Valor (col. Z)" style="width:100px;padding:3px 6px;font-size:11px;border-radius:4px;border:1px solid #4b5563;background:#1f2937;color:#fff;">
                                <button @click="if(valor!=='') { $wire.lancarCupomPlataforma({{ $venda->id_venda }}, valor); open=false; }" type="button"
                                    style="background:#059669;color:#fff;padding:3px 8px;font-size:11px;border-radius:4px;border:none;cursor:pointer;">✓</button>
                                <button @click="open=false" type="button" style="color:#9ca3af;font-size:10px;cursor:pointer;background:none;border:none;">✖</button>
                            </span>
                        </div>
